...
export function PrimaryColumn({ primaryColumn,title, selectRow }:PrimaryColumnProps) {
  return (
    <div className="grid-col grid-col--primary">
      ...
      {primaryColumn.map((cell:GridPrimaryCell, cellIndex:number) => (
        <div className="grid-cell" key={cellIndex} onClick={() => { selectRow(cellIndex)}}>
...
export default function GridTable({grid, addNewColumn, hydrateCell, selectRow }:Props) {
  return (
    <div className="grid">
      <div className="grid-title">{grid.title}</div>
      <div className="grid-columns">
        <PrimaryColumn primaryColumn={grid.primaryColumn} title={grid.primaryColumnType} selectRow={selectRow} />
        {grid.columns.map((column:GridCol, rowIndex:number) => (
          <div className="grid-col" key={rowIndex}>
            ...
            {column.cells.map((cell:GridCell, cellIndex:number) => (
              <Cell cell={cell} key={cellIndex} hydrate={hydrateCell} />
            ))}
          </div>
        ))}
